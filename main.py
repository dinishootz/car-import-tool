...
def extract_olx_price():
    """Extrai o preço mais baixo do OLX"""
    olx_url = "https://www.olx.pt/automoveis/"
    headers = {'User-Agent': 'Mozilla/5.0'}
    response = requests.get(olx_url, headers=headers)
    
    if response.status_code == 200:
        soup = BeautifulSoup(response.text, 'html.parser')
        prices = soup.find_all(text=re.compile(r'\d{1,3}(?:\.\d{3})*,\d{2}'))
        sorted_prices = sorted([float(p.replace('.', '').replace(',', '.')) for p in prices])
        return sorted_prices[0] if sorted_prices else "Sem dados"
...
def analyze_car(url: str):
    ...
    # Comparação de preços com OLX
    car_data['lowest_olx_price'] = extract_olx_price()
